...
const CartItemCounter: FC<CartItemCounterProps> = ({ productId, count }) => {
  ...
  const [
    setCount,
    { loading, error },
  ] = useMutation<setProductCountInCart, setProductCountInCartVariables>(
    SET_PRODUCT_COUNT_IN_CART,
    {
      update: (cache, { data }) => {
        const productCount = data?.changeProductCountInCart?.cartItems
          ?.reduce((acc, item) => acc + (item?.count ?? 0), 0) as number

        cache.writeQuery<getCartCount>({
          query: GET_CART_ITEMS_COUNT,
          data: {
            countProductsInCart: productCount,
          },
        })
        ...
        cartItems?.forEach((item, index) => {
          const modifyIndex = data?.changeProductCountInCart?.cartItems
            ?.findIndex((i) => i?.id === item?.id) as number
          if (index >= 0) {
            // @ts-ignore
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value > 0) {
      setTimer(setTimeout(async () => set(value), 500))
    } else {
      setTimer(setTimeout(async () => set(1), 500))
    }
  }
