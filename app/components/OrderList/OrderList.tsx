...
const OrderList: FC<OrderListProps> = ({ jwt }) => {
  ...
  const orders = useMemo(() => data?.me?.user?.orders
    ?.filter((item) => item?.invoice?.search(filter) !== -1).map((item) => item && (
            ...
            <Grid container spacing={2} direction="column">
              <Grid item className={isSmallPhone ? styles.scaleTable : ''}>
                <Table aria-label="simple table">
                  ...
                  <TableBody>
                    {item.saleProductData?.map((p) => (
                      <TableRow
                        key={p?.id}
                        className={styles.hoverRow}
                        onClick={async () => router.push('/products/[id]', `/products/${p?.product?.id}`)}
                      >
              ...
              <Grid item xs={12} className={styles.alignRight}>
                <Typography variant="subtitle1">
                  ...
                  {' '}
                  {item.saleProductData?.reduce(
                    (acc, p) => acc + (p?.count ?? 0) * (p?.sellingPrice ?? 0),
                    0,
                  )}
                  {' '}
                  USD
  ...
  if (error) {
    return (
      <Grid container justifyContent={loading ? 'center' : 'space-evenly'} spacing={1}>
