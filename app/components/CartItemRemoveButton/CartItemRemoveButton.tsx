...
const CartItemRemoveButton: FC<CartItemRemoveButtonProps> = ({ productId }) => {
  ...
  const [
    removeItem,
    { loading, error },
  ] = useMutation<deleteProductFromCart, deleteProductFromCartVariables>(DELETE_PRODUCT_FROM_CART, {
    update: (cache, { data }) => {
      ...
      const count = data?.deleteProductFromCart?.cartItems
        ...
        data: {
          getCart: {
            ...
            cartItems: cart?.cartItems
              ?.filter((item) => ids
                .includes(item?.id as string)) as (getCart_getCart_cartItems | null)[] | null,
          },
        },
      })
